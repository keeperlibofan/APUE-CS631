...
调用函数sigprocmask可以检测和更改，或同时进行检测和更改进程的信号屏蔽字。
sigprocmask是单线程进程决定的，处理多线程信号是另外一个函数。

`int	sigprocmask(int set, const sigset_t * __restrict, sigset_t * __restrict);`

set是个空指针，不改变信号的屏蔽字，how值也没有意义。
进程当前的信号屏蔽字是通过oset来返回的。

第一个参数
+ SIG_BLOCK:   并集
+ SIG_UNBLOCK: 补集的交集
+ SIG_SETMASK: 直接将信号屏蔽是set指向的值
会有阻塞行为，因为要遍历检查是否有未决信号。

#### sigpending 函数

返回一个信号集，调用进程的信号是阻塞不能递送的
...
问题：有一个问题。当捕捉到一个信号的时候，进入信号处理函数，此时当前的信号被自动的加入
了进程的信号屏蔽字中。这阻止了后来产生的这种信号中断处理函数。如果用longjmp跳出信号处理函数
，那么对此进程的信号屏蔽字会发生什么。

在信号处理程序中进行非局部专业的时候应当使用这两个函数。

多加了一个参数 `int savemask`
